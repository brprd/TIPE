...
class Boat:
    ...
    def centrer(self): #centre la carte sur le bateau lui-même
        ...
        plt.xlim(donnes_instant[1]-e, donnes_instant[1]+e)
        plt.ylim(donnes_instant[0]-e,donnes_instant[0]+e)
    def next(self):
        self.__compteur+=1
    ...
    def kalman(self, compteur):
        global d
        Q = np.array([[0.1*BG(BG_seed),0,0,0],[0,0.1*BG(BG_seed),0,0],[0,0,0.00001*BG(BG_seed),0],[0,0,0,0.00001*BG(BG_seed)]]) #matrice de covariance du bruit du modèle physique
